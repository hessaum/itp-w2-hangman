...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    :param remaining_misses: How many misses are left before user loses
    """
    if _check_win(answer_word, masked_word) or _check_lost(remaining_misses):
	return True
    else:
	return False
...
def start_new_game(word_list, answer_word=None):
	...
	word = _get_random_word(word_list)
    else:
	word = answer_word
...
def guess_letter(game, letter):
	...
	for i in range(len(game['answer_word'])):
	    if letter == game['answer_word'][i]:
		game['masked_words'] = game['masked_word'][:i] + letter + game['masked_word'][i + 1:]
    else:
